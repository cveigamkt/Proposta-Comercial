...
// Supabase envs: avoid reserved prefix SUPABASE_ by using SB_*
const SB_URL = Deno.env.get("SB_URL");
const SB_SERVICE_ROLE_KEY = Deno.env.get("SB_SERVICE_ROLE_KEY");

const sb = SB_URL && SB_SERVICE_ROLE_KEY
...
serve(async (req: Request) => {
  try {
    ...
    if (!toList.length) {
      return new Response(JSON.stringify({ error: "Missing recipient(s)" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (!sendResp.ok) {
      ...
      try {
        detail = await sendResp.json();
      } catch (_) {
        try {
          detail = await sendResp.text();
        } catch (_) {
          detail = null;
        }
      }
      const body = { error: detail, status: sendResp.status };
