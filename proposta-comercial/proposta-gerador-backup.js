...
function calcularValores() {
    ...
    if (descontoDescricao && descontoValorInput) {
        ...
        if (!isNaN(valorDesconto) && valorDesconto > 0) {
            ...
            // Elementos removidos - desconto customizado será mostrado apenas nos cards
        }
    }
...
document.getElementById('servicoTrafegoPago').addEventListener('change', function() {
    ...
    if (plano === 'nao-se-aplica') {
        ...
        if (dados.investimentoMax) {
            limiteTexto.innerHTML = `<strong>⚠️ Importante:</strong> Este é o valor que o cliente investirá mensalmente em anúncios (Meta Ads, Google Ads, etc.). Este valor NÃO inclui nosso valor mensal de gestão.<br><strong>Limite deste plano:</strong> ${formatarMoeda(dados.investimentoMin)} - ${formatarMoeda(dados.investimentoMax)}`;
        } else {
            limiteTexto.innerHTML = `<strong>⚠️ Importante:</strong> Este é o valor que o cliente investirá mensalmente em anúncios (Meta Ads, Google Ads, etc.). Este valor NÃO inclui nosso valor mensal de gestão.<br><strong>Limite deste plano:</strong> Acima de ${formatarMoeda(dados.investimentoMin)}`;
        }
...
async function criarPropostaNaPlanilha(dados) {
    ...
    const dadosProposta = {
        ...
        descontosTotais: 'Calculado na visualização',
        valorMensal: 'Calculado na visualização',
...
function calcularDescontosTotais() {
    ...
    const totalAposDesconto = total - descontoCustomizado;
    const descontoAdicional = totalAposDesconto * 0.05;
    const descontoTotal = descontoCustomizado + descontoAdicional;
...
function calcularValorTotal() {
    ...
    // Aplicar descontos (usando 6 meses como padrão)
    const descontoRecorrencia = total * 0.10; // 10% para 6 meses
    const totalAposRecorrencia = total - descontoRecorrencia;
    const descontoAdicional = totalAposRecorrencia * 0.05;
    const valorFinalMensal = totalAposRecorrencia - descontoAdicional;
    
    return formatarMoeda(valorFinalMensal * 6); // Estimativa semestral
}
